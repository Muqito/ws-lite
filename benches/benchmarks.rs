...
use ws_lite::dataframe::mask_data;
...
fn messages(c: &mut Criterion) {
    let mut group = c.benchmark_group("WriteMessage");

    group.bench_function("message_to_tcp_write_data", |b| {
        b.iter(|| {
            let message = Message::Text(String::from("a"));
            black_box(WriteMessage::from(message))
        })
    });

    group.finish()
}
fn masking_key(c: &mut Criterion) {
    let mut group = c.benchmark_group("masking_key");
    group.bench_function("regular mask", |b| {
        let mask: [u8; 4] = [0, 1, 0, 1];
        let mut data: [u8; 503] = {
            [
                71, 69, 84, 32, 47, 32, 72, 84, 84, 80, 47, 49, 46, 49, 13, 10, 72, 111, 115, 116,
                58, 32, 91, 58, 58, 49, 93, 58, 51, 51, 51, 51, 13, 10, 85, 115, 101, 114, 45, 65,
        ...
        b.iter(|| mask_data(&mut data, mask));
    });

    group.finish()
}

criterion_group!(benches, ws_headers, messages, masking_key);
